...
| 9 | `duixueqiu_video_materials` | 视频号素材表 | SERIAL | - |
...
```mermaid
erDiagram
    users ||--o{ rewrite_history : "创建"
    users ||--o{ publish_tasks : "创建"
    users ||--o{ follow_circle_tasks : "创建"
    users ||--o{ duixueqiu_accounts : "拥有"
    users ||--o{ duixueqiu_video_materials : "拥有"

    wechat_articles ||--o{ rewrite_history : "被转写"

    rewrite_history ||--o{ publish_tasks : "生成"
    ...
    wechat_articles {
        ...
        TEXT content "文章正文"
        TEXT[] images "图片URL数组"
        TIMESTAMP publish_time "发布时间"
        VARCHAR author "作者"
        VARCHAR url "原文链接"
        VARCHAR account_name "公众号名称"
        VARCHAR account_id "公众号ID"
        VARCHAR status "处理状态"
        TEXT rewritten_content "改写后内容"
        TIMESTAMP created_at "创建时间"
        TIMESTAMP updated_at "更新时间"
    }
    ...
    rewrite_history {
        ...
        TEXT[] original_images "原始图片"
        TEXT rewritten_content "转写后内容"
        TEXT[] selected_images "选择的图片"
        VARCHAR coze_workflow_id "Coze工作流ID"
        VARCHAR status "转写状态"
    ...
    publish_tasks {
        UUID id PK "任务ID"
        UUID user_id FK "用户ID"
        UUID rewrite_id FK "转写记录ID"
        VARCHAR task_title "任务标题"
        TEXT content "发布内容"
        TEXT[] images "图片URL数组"
        VARCHAR wechat_account "微信账号"
        TIMESTAMP publish_time "发布时间"
        BOOLEAN is_immediate "是否立即发布"
        INTEGER random_delay_minutes "随机延迟分钟"
        VARCHAR status "任务状态"
        VARCHAR duixueqiu_task_id "堆雪球任务ID"
        TEXT error_message "错误信息"
        TIMESTAMP created_at "创建时间"
        TIMESTAMP updated_at "更新时间"
    }
    ...
    follow_circle_tasks {
        SERIAL id PK "任务ID"
        UUID user_id FK "用户ID"
        VARCHAR task_group_id "任务组ID"
        INT circle_index "第几条跟圈"
        TEXT content "朋友圈内容"
        JSONB images "图片列表JSON"
        TIMESTAMP publish_time "发布时间"
        BOOLEAN delete_previous "是否删除上一条"
        VARCHAR previous_title "上一条标题"
        VARCHAR status "任务状态"
        TEXT error_message "错误信息"
        TIMESTAMP created_at "创建时间"
        TIMESTAMP updated_at "更新时间"
    ...
    delete_circle_tasks {
        SERIAL id PK "任务ID"
        VARCHAR task_group_id "任务组ID"
        INTEGER circle_index "第几条朋友圈"
        VARCHAR delete_title "要删除的标题"
        TEXT delete_content "朋友圈内容"
        TIMESTAMP delete_time "删除时间"
        VARCHAR status "任务状态"
        TEXT error_message "错误信息"
        TIMESTAMP created_at "创建时间"
        TIMESTAMP updated_at "更新时间"
    ...
    duixueqiu_accounts {
        SERIAL id PK "账号ID"
        UUID user_id FK "用户ID"
        VARCHAR account_name "账号名称"
        VARCHAR username "堆雪球用户名"
        TEXT password "堆雪球密码"
        BOOLEAN is_default "是否默认账号"
        VARCHAR status "账号状态"
        TIMESTAMP created_at "创建时间"
        TIMESTAMP updated_at "更新时间"
    }
    ...
    duixueqiu_video_materials {
        SERIAL id PK "素材ID"
        UUID user_id FK "用户ID"
        VARCHAR author_name "发布者名称"
        TEXT content_desc "内容描述"
        INT material_index "索引位置"
        INT page_number "所在页码"
        VARCHAR group_name "素材分组"
...
```mermaid
graph TD
    A[users<br/>用户表] --> B[rewrite_history<br/>转写历史]
    A --> C[publish_tasks<br/>发布任务]
    A --> D[follow_circle_tasks<br/>跟圈任务]
    A --> E[duixueqiu_accounts<br/>堆雪球账号]
    A --> F[duixueqiu_video_materials<br/>视频号素材]
...
```mermaid
graph LR
    A[wechat_articles<br/>公众号文章] --> B[rewrite_history<br/>AI转写]
    B --> C[publish_tasks<br/>发布任务]
...
```mermaid
graph LR
    A[用户创建跟圈任务] --> B[follow_circle_tasks<br/>跟圈任务表]
    B --> C{是否需要删除上一条?}
    C -->|是| D[delete_circle_tasks<br/>删除任务表]
    C -->|否| E[直接发布]
