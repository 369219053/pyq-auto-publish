...
### Q6: 脚本2好友搜索只找到群聊,找不到个人好友? (重要!)

**问题**: 脚本2组合发送时,搜索好友"小灵(沪港纪老板)"失败,日志显示只找到18个群聊元素,没有找到个人好友

**问题现象**:
- 选择好友"小灵(沪港纪老板)"和"沪港纪老板"
- "沪港纪老板"发送成功(文字+视频+链接)
- "小灵(沪港纪老板)"发送失败,提示"未找到好友"
- 日志显示: `所有元素: [沪港纪老板、Christine、房外房冯冰冰Icey1561, ...]`
- 所有元素都包含"、"分隔符,说明都是群聊
...
- 好友名称: "小灵(沪港纪老板)"
- 代码分割为: `["小灵", "沪港纪老板"]`
- 之前逻辑: 选择**最长的片段**"沪港纪老板"(5个字符)
- 正确逻辑: 应该选择**第一个片段**"小灵"(括号外的内容)

**原因2: 群聊过滤逻辑问题**
- 代码添加了"好友"分隔符过滤,但过滤后好友元素数量为0
- 可能是DOM位置比较逻辑有问题
- 需要添加详细调试日志查看过滤过程
...
```typescript
// wechat-reach.service.ts (第992-1015行)
...
```typescript
// wechat-reach.service.ts (第1028-1071行)
...
**预防措施**:
- 好友名称格式: 优先使用括号外的内容作为搜索关键词
- 添加详细调试日志,便于排查问题
- 测试各种好友名称格式: "小灵(沪港纪老板)"、"微博-杨女士-购房"等

**相关文档**:
- [脚本2文档 - 群聊过滤](./脚本2-微信好友触达.md#核心特性)
...
### Q21: 新用户绑定堆雪球账号后,左侧微信号列表为空?

**问题**: 新用户首次使用脚本2功能时,打开"好友列表管理"弹窗,左侧微信号列表为空,右侧显示"请先选择左侧的微信号"

**问题现象**:
- ✅ 已在"系统设置 → 堆雪球账号"中成功绑定账号
- ✅ 进入"脚本2 - 微信好友触达"页面
- ✅ 点击"好友列表管理"卡片
- ❌ 左侧微信号列表为空
- ❌ 右侧显示"请先选择左侧的微信号"
...
新用户绑定堆雪球账号后,系统**没有自动同步微信号列表**,需要手动触发同步。

**解决方案**:

**步骤1: 点击"📱 同步微信号"按钮**

在"好友列表管理"弹窗中,左侧微信号列表顶部有一个蓝色的"📱 同步微信号"按钮,点击它。
...
**本地测试时查看Puppeteer操作过程**:

如果想看到Puppeteer浏览器的操作过程,可以:
...
1. **同步失败**: 检查堆雪球账号密码是否正确
2. **同步超时**: 网络较慢时可能需要等待更长时间
3. **微信号数量不对**: 检查堆雪球账号中是否有所有微信号

---

### Q22: 同步微信号时提取的微信号不正确?

**问题**: 点击"同步微信号"按钮后,提取的微信号名称不正确或数量不对
...
Puppeteer提取微信号时,使用的CSS选择器可能不正确,或者页面结构发生了变化。
...
如果发现CSS选择器不正确,需要修改代码:

```typescript
// 文件: pyq-backend/src/automation/wechat-reach.service.ts
// 方法: getWechatAccounts()

// 可能需要修改的选择器:
const items = document.querySelectorAll('.wechat-account-list > .item');  // 微信号列表项
const nameDiv = item.querySelector('.name');  // 微信号名称元素
const name = nameDiv.textContent?.trim();  // 提取文本内容
...
1. **类名变化**: 堆雪球更新后,CSS类名可能改变
2. **结构变化**: HTML结构可能调整
3. **动态加载**: 微信号列表可能是动态加载的,需要等待

**解决方案**:

1. **使用更稳定的选择器**: 优先使用`data-*`属性或`id`
2. **增加等待时间**: 确保页面完全加载
3. **使用多种提取方式**: 同时尝试`.textContent`和`.getAttribute('title')`

---

### Q23: 同步好友时提示"请先选择左侧的微信号"?

**问题**: 点击"同步好友"按钮时,提示"请先选择左侧的微信号"

**问题原因**:

没有选中任何微信号,或者选中状态没有正确保存。

**解决方案**:

**步骤1: 确保已同步微信号**

如果左侧微信号列表为空,先点击"📱 同步微信号"按钮。

**步骤2: 选择微信号**

- 在左侧微信号列表中,点击需要同步的微信号
- 微信号会高亮显示(蓝色背景)
- 可以选择多个微信号
...
- **全选**: 点击"✅ 全选"按钮,选中所有微信号
- **取消**: 点击"❌ 取消"按钮,取消所有选择
- **同步所有**: 点击"同步所有"按钮,同步所有微信号的好友

---

### Q24: 好友同步速度很慢?

**问题**: 同步好友列表时,速度非常慢,需要等待很长时间

**问题原因**:

1. **好友数量多**: 每个微信号可能有几千个好友
2. **滚动加载**: 需要不断滚动页面加载更多好友
3. **网络延迟**: 堆雪球服务器响应较慢
4. **等待时间**: 为了避免被检测,增加了等待时间
...
1. **减少等待时间**: 如果网络较快,可以减少等待时间
2. **分批同步**: 不要一次同步所有微信号,分批进行
3. **后台同步**: 同步过程在后台进行,不影响其他操作

**预计同步时间**:

- **单个微信号(1000好友)**: 约5-10分钟
- **单个微信号(5000好友)**: 约20-30分钟
- **多个微信号**: 按顺序同步,时间累加
...
### Q25: 跟圈任务并发冲突导致发布失败? (重要!)

**问题**: 同时运行多个跟圈任务时,出现"未找到匹配的素材"、"浏览器突然关闭"等错误
...
**根本原因**: 多个跟圈任务并发执行时,同时操作同一个堆雪球账号,导致冲突

**详细分析**:

1. **并发场景**:
   - 任务A正在发布第一条朋友圈(需要3-5分钟)
   - 任务B的定时任务触发(1分钟后)
   - 任务B也开始登录堆雪球,打开浏览器
   - 两个Puppeteer实例同时操作同一个账号
...
3. **之前的锁机制问题**:
   ...
   const job = schedule.scheduleJob(publishTime, async () => {
     ...
     await this.publishCircle(task);  // 发布朋友圈
   ...
   **问题**: 第一条朋友圈不经过定时任务,直接调用`publishCircle()`,绕过了锁!

**解决方案**:

**已在V6.0版本修复,将互斥锁移到`publishCircle()`方法的最外层**
...
private async publishCircle(task: any): Promise<void> {
  ...
  try {
    ...
    // 所有重试都失败,释放锁
    this.logger.log(`🔓 发布失败,释放执行锁`);
    this.taskExecutionLock.delete(lockKey);
    throw lastError;
  } catch (error) {
    // 异常时也释放锁
    this.logger.log(`🔓 异常释放执行锁`);
    this.taskExecutionLock.delete(lockKey);
    throw error;
  }
}
...
# 应该看到:
🔒 已获取执行锁,开始发布任务: task_A_1
📤 第1次尝试发布朋友圈: 自动跟圈1
✅ 发布成功,释放执行锁: task_A_1

⏰ 定时任务触发: task_A_2
⚠️ 有其他跟圈任务正在执行,等待完成...  ← 任务B在等待
⏳ 等待其他任务完成...

🔒 已获取执行锁,开始发布任务: task_A_2
📤 第1次尝试发布朋友圈: 自动跟圈2
✅ 发布成功,释放执行锁: task_A_2

🔒 已获取执行锁,开始发布任务: task_B_1  ← 任务B开始执行
```
...
**相关问题**:
- 如果遇到停止任务失败,参考下方的"Q26: 停止任务失败"
- 如果遇到任务卡住,检查锁是否正确释放

---

### Q26: 停止任务失败,提示"Cannot read properties of null"? (重要!)

**问题**: 点击"停止任务"按钮时,提示"停止失败: Cannot read properties of null (reading 'cancel')"

**问题现象**:
- ✅ 跟圈任务正在执行
- ✅ 点击"停止任务"按钮
- ❌ 前端提示停止失败
- ❌ 后端日志显示: `Cannot read properties of null (reading 'cancel')`
- ❌ 任务仍在继续执行

**问题原因**:

**根本原因**: 当任务还没有创建定时任务时,`scheduledJobs` Map中的job对象可能是null
...
1. **停止任务的流程**:
   ...
   this.scheduledJobs.forEach((job, key) => {
     if (key.startsWith(taskGroupId)) {
       job.cancel();  // ❌ job可能是null!
     }
   });
   ```

2. **什么时候job会是null?**:
   - 第一条朋友圈正在发布中(还没有创建定时任务)
   - 定时任务已经执行完毕(job已被删除)
   - 任务创建失败(job创建失败但key已存在)
...
1. **`stopTask()`方法** - 停止任务时的定时任务取消
2. **`cancelFollowCircleTasks()`方法** - 取消任务组时的定时任务取消
...
**相关问题**:
- 如果遇到任务无法停止,检查`stoppedTasks` Map是否正确设置
- 如果遇到定时任务重复执行,检查job是否正确删除
...
### Q25: 脚本2发送图片时确定按钮无法点击? (重要!)

**问题**: 使用脚本2发送图片消息时,图片上传成功,但"发送文件"确认框的"确定"按钮无法被点击

**问题现象**:
- ✅ 图片成功上传到堆雪球系统
- ✅ 弹出"发送文件"确认框
- ❌ Puppeteer无法找到或点击"确定"按钮
- ❌ 日志显示: `📋 Element UI对话框: 0 个`
- ❌ 消息未发送,浏览器继续等待

**问题原因**:

**堆雪球的"发送文件"确认框不是Element UI的对话框结构**
...
2. **实际情况**:
   - 手动上传图片后弹出的确认框标题是"发送图片"
   - Puppeteer上传图片后弹出的确认框标题是"发送文件"
   - 这个确认框**不是**`.el-dialog__wrapper`结构
   - 可能是原生`<dialog>`标签或其他自定义弹窗

3. **为什么找不到?**:
   - 代码只查找`.el-dialog__wrapper`中的确定按钮
   - 实际对话框不是这个结构
   - 导致无法找到确定按钮

**解决方案**:

**已在V5.9版本修复,使用通用的按钮查找策略**

**修复内容**:

```typescript
// ❌ 错误方式: 只查找Element UI对话框
const dialogs = document.querySelectorAll('.el-dialog__wrapper');
...
// ✅ 正确方式: 查找所有可见的"确定"按钮,选择z-index最大的
...
1. ✅ **不依赖特定的对话框结构** - 适用于任何类型的确认框
2. ✅ **智能选择最上层按钮** - 通过z-index确保点击正确的按钮
3. ✅ **检查可见性** - 确保按钮及其父元素都可见
4. ✅ **支持多种文本格式** - "确定"或"确 定"(中间有空格)
...
1. **不要假设对话框结构** - 不同系统的对话框实现方式不同
2. **使用通用的查找策略** - 查找所有按钮,而不是特定结构中的按钮
3. **z-index判断** - 确保点击最上层的按钮,避免点击错误的按钮
4. **可见性检查** - 检查按钮及其父元素的可见性

**相关问题**:
- 如果遇到其他确认框无法点击,使用相同的通用查找策略
- 如果遇到按钮文本不同,修改文本匹配条件
...
**问题**: 想给好友发送多条不同的文字消息,而不是一条长消息

**功能说明**:

脚本2支持**添加多条独立的文字消息**,每条消息会单独发送,并且保留原有的换行符。
...
- ✅ **每条消息独立发送** - 不是一次性发送所有内容
- ✅ **保留换行符** - 每条消息内的换行符不会被拆分
- ✅ **自动间隔** - 每条消息间自动添加3-8秒随机延迟
- ✅ **防重复发送** - 每条消息独立记录,已发送的自动跳过
- ✅ **支持变量** - 每条消息都支持`{昵称}`变量替换
- ✅ **随机表情** - 每条消息自动添加1-2个随机表情

**注意事项**:

1. **最多10条消息** - 达到上限后"添加文字消息"按钮会禁用
2. **每条消息独立计费** - 如果有发送限制,每条消息都会计数
3. **删除操作** - 鼠标悬停在消息卡片上会显示删除按钮

---

### Q26: 如何发送多张图片?

**问题**: 想给好友发送多张图片,每张图片单独发送

**功能说明**:

脚本2支持**上传多张图片**(最多9张),每张图片会**单独发送**,而不是一次性发送所有图片。

**使用步骤**:

**步骤1: 勾选"图片消息"**

在"选择内容类型"中,勾选"🖼️ 图片消息"。

**步骤2: 点击"选择图片"按钮**

- 点击橙色的"📁 选择图片"按钮
- 可以一次性选择多张图片(最多9张)
- 支持PNG/JPG格式,每张最大15MB
...
- ✅ **每张图片单独发送** - 不是一次性发送所有图片
- ✅ **防重复发送** - 每张图片独立记录,已发送的自动跳过
- ✅ **自动间隔** - 如果启用随机延迟,每张图片间会自动添加延迟
- ✅ **图片存储** - 图片存储在Supabase Storage,自动清理7天前的图片

**注意事项**:

1. **最多9张图片** - 达到上限后"选择图片"按钮会禁用
2. **每张图片单独计费** - 如果有发送限制,每张图片都会计数
3. **图片大小限制** - 每张图片最大15MB
4. **自动清理** - 系统会在每天凌晨3点自动清理7天前的图片

---

### Q27: 组合发送时,多条文字消息和多张图片如何配合?

**问题**: 想同时发送多条文字消息、多张图片、视频号、链接,它们的发送顺序是什么?
...
- 如果启用了"随机延迟",每条消息/每张图片之间会有5-15秒的随机延迟
- 文字消息之间固定有3-8秒的间隔(即使没有启用随机延迟)

**防重复机制**:

- 每条消息/每张图片都会独立记录
- 如果某条消息已发送过,会自动跳过
- 如果任务暂停后恢复,已发送的内容不会重复发送

**适用场景**:

- **产品推广**: 文字介绍 + 产品图片 + 视频号演示
- **活动通知**: 活动文案 + 活动海报 + 报名链接
- **内容分享**: 文章摘要 + 文章链接 + 配图

---

### Q28: 脚本2好友全选功能失败,提示"Bad Request"? (重要!)

**问题**: 在脚本2页面选择微信号后,点击"✅ 全选"按钮,提示"全选失败: Bad Request"
...
- ✅ 右侧显示该微信号的好友列表
- ❌ 点击"✅ 全选"按钮
- ❌ 浏览器控制台报错: `Bad Request`
- ❌ 好友未被全选

**问题原因**:

**原因1: 好友ID类型不匹配**
- 前端发送的`friendIds`是字符串数组
- 后端API期望的是`number[]`数组
- 导致参数验证失败,返回400错误
...
```typescript
// 前端代码
const friendIds = filteredFriends.value.map((f) => f.id);  // 可能是字符串
console.log('好友ID类型检查:', typeof friendIds[0]);  // "string"
...
{
  "userId": "xxx",
  "friendIds": [328724, 328551, 328720, ...],  // 6510个数字
  "isSelected": true
}
// 请求体大小约 50-100 KB,可能超过限制
```
...
```typescript
// script2.vue
const friendIds = filteredFriends.value.map((f) => Number(f.id));  // ✅ 转换为数字
```
...
const selectAllFriends = async () => {
  ...
  try {
    // 获取当前筛选出的好友ID列表 - 确保转换为数字类型
    const friendIds = filteredFriends.value.map((f) => Number(f.id));

    console.log('准备全选好友:', friendIds.length, '个');

    // 🔧 分批处理,避免请求体过大(每批1000个)
    ...
    console.log(`分成 ${batches.length} 批处理,每批最多 ${batchSize} 个`);

    // 依次处理每一批
    for (let i = 0; i < batches.length; i++) {
      const batch = batches[i];
      console.log(`处理第 ${i + 1}/${batches.length} 批 (${batch.length}个好友)...`);

      const response = await fetch('/api/automation/friends/batch-select', {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          userId,
          friendIds: batch,  // 每批最多1000个
          isSelected: true,
        }),
      });

      if (!response.ok) {
        throw new Error(`第${i + 1}批处理失败: ${response.status}`);
      }

      const result = await response.json();
      if (!result.success) {
        throw new Error(result.message || `第${i + 1}批处理失败`);
      }

      console.log(`✅ 第 ${i + 1}/${batches.length} 批处理成功`);
    }

    // 更新本地状态
    filteredFriends.value.forEach((f) => (f.is_selected = true));
    triggerRef(friends);
    message.success(`已全选 ${friendIds.length} 个好友`);
  } catch (error) {
    console.error('全选失败:', error);
    message.error(`全选失败: ${error.message}`);
  }
};
```
...
```bash
# 1. 刷新浏览器 (Ctrl+Shift+R 或 Cmd+Shift+R)
# 2. 访问 https://autochat.lfdhk.com/
# 3. 进入"脚本2 - 微信好友触达"
# 4. 点击"同步好友"
# 5. 选择"1号机"(或其他微信号)
# 6. 点击右侧好友列表的"✅ 全选"按钮
# 7. 查看浏览器控制台,应该看到分批处理的日志
# 8. 所有好友应该被成功全选
```

**技术要点**:

1. **类型转换**: 使用`Number()`确保ID是数字类型
2. **分批处理**: 每批1000个,避免请求体过大
3. **错误处理**: 每批独立处理,失败时提示具体批次
4. **进度反馈**: 控制台输出详细的处理进度
