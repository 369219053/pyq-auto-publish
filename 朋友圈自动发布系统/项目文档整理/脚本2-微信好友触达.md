...
批量发送消息给所有微信好友,支持**四种消息类型**:文字消息、视频号卡片、链接卡片、图片消息,以及**组合发送**(同时发送多种类型)。支持多账号轮询、智能间隔、时间控制、消息个性化、**防重复发送**、**暂停/恢复**等功能。适用于产品推广、活动通知等场景。

### ✨ 核心特性

- **防重复发送机制** - 基于SHA-256哈希值,自动跳过已发送的消息
- **暂停/恢复功能** - 可随时暂停任务,关闭浏览器,稍后恢复继续发送
- **组合发送** - 一次任务可同时发送文字+视频号+链接+图片
- **智能去重** - 每种消息类型独立记录,精准控制发送状态
- **随机延迟** - 每条消息发送前可添加5-15秒随机延迟,模拟真人操作,防止被封号
- **智能表情** - 文字消息自动添加1-2个随机表情到首尾,2500万种组合,几乎不重复
- **双重匹配** - 素材匹配支持URL和文字双重匹配,提高成功率
- **群聊过滤** - 自动识别并过滤群聊,只发送给个人好友
...
- **功能**: 发送纯文本消息给好友
- **特性**:
  - 🆕 **支持多条文字消息** - 可添加最多10条独立的文字消息,每条单独发送
  - 支持`{昵称}`变量个性化
  - 自动添加随机表情(1-2个到首尾)
  - 100个表情池,2500万种组合
  - 防重复基于核心文案(去除表情后)
...
- **功能**: 发送图片给好友
- **图片来源**: 本地上传或URL链接
- **数据库表**: `follow_circle_tasks` (存储图片URL)
- **特性**:
  - 支持多张图片(最多9张)
  - 🆕 **每张图片单独发送** - 不再一次性发送所有图片,每张图片独立发送并记录
  - 支持预览和删除
...
### 5️⃣ 组合发送
- **功能**: 一次任务同时发送多种类型的消息
- **支持组合**: 文字+视频号+链接+图片(任意组合)
- **防重复机制**: 每种类型独立记录,已发送的类型自动跳过
- **暂停/恢复**: 支持暂停任务,稍后恢复继续发送
- **适用场景**: 综合营销、多内容推广
...
- **`pyq-backend/src/automation/wechat-reach.service.ts`**
  - `startWechatReachTask()` - 主流程函数(文字消息)
  - `startCombinedReachTask()` - 组合发送主流程
  ...
  - `sendCombinedContents()` - 发送组合消息
  - `checkMessageSent()` - 检查消息是否已发送(防重复)
  - `recordMessageSent()` - 记录消息发送历史
  - `calculateMessageHash()` - 计算消息内容哈希值
  - `pauseTask()` - 暂停任务
  - `resumeTask()` - 恢复任务
...
- **`pyq-backend/src/automation/automation.controller.ts`**
  - `POST /api/automation/script2/wechat-reach` - 开始任务(单一类型)
  - `POST /api/automation/script2/combined-reach` - 开始组合发送任务
...
```typescript
// 同步微信号列表
POST /api/automation/wechat-accounts/sync
Content-Type: application/json
...
// 获取微信号列表(从数据库)
GET /api/automation/wechat-accounts?userId={userId}
...
```typescript
// 同步好友列表
POST /api/automation/friends/sync
Content-Type: application/json

{
  "userId": "用户UUID",
  "wechatAccountNames": ["微信号1", "微信号2"]  // 可选,不传则同步所有
}
...
// 获取好友列表
GET /api/automation/friends?userId={userId}
...
**数据库表**: `message_send_history`
- `user_id` (UUID) - 用户ID
- `friend_id` (BIGINT) - 好友ID
- `message_type` (TEXT) - 消息类型(text/video/link/image/combined)
- `message_content_hash` (TEXT) - SHA-256哈希值
- `message_content` (JSONB) - 完整消息内容
...
**工作流程**:
1. 发送前调用`checkMessageSent()`检查是否已发送
2. 如果已发送,跳过该好友
3. 如果未发送,发送消息后调用`recordMessageSent()`记录
4. 组合发送时,每种类型独立检查和记录
...
**核心原理**: 利用防重复发送机制,暂停后重新调用主函数,自动跳过已发送的消息
...
async pauseTask(): Promise<void> {
  ...
  // 保留任务参数,用于恢复
  // this.currentTaskParams 包含所有任务配置
}
...
**关键点**:
- 暂停时保存`currentTaskParams`(任务配置)
- 恢复时重新调用主函数,传入相同参数
- 依赖`checkMessageSent()`自动跳过已发送的消息
- 无需手动记录进度,完全依赖数据库记录
...
**核心原理**: 在每条消息发送前添加随机延迟,模拟真人操作,避免被检测为机器人
...
**配置参数**:
- `enabled` (boolean) - 是否启用随机延迟
- `minDelay` (number) - 最小延迟时间(秒),默认3秒
- `maxDelay` (number) - 最大延迟时间(秒),默认10秒

**前端UI**:
- 位置: "发送策略配置"卡片
- 控件: 复选框(启用/禁用) + 两个数字输入框(最小/最大延迟)
- 实时提示: 显示当前延迟范围(如"每条消息间隔 3-10 秒")

**使用场景**:
- 大批量发送时,避免被微信检测为机器人
- 模拟真人操作,提高发送成功率
- 可根据需要调整延迟范围(如高峰期增加延迟)

---

### 4. 双重匹配机制

**核心原理**: 素材匹配支持URL和文字双重匹配,提高匹配成功率,避免因素材库不同步导致发送失败

#### 视频号素材双重匹配

**优先级**: 缩略图URL精确匹配 > 作者名+描述文字匹配
...
**优先级**: 标题+公众号名称精确匹配 > 标题前30字符+公众号名称模糊匹配
...
**优势**:
- **提高成功率**: 即使缩略图URL变化,仍可通过文字匹配
- **容错性强**: 支持模糊匹配,避免因标题微小差异导致失败
- **无需频繁同步**: 减少对素材库同步的依赖
...
1. 进入"脚本2 - 微信好友触达"页面
2. 点击"好友列表管理"卡片
3. 在弹出的对话框中,点击左侧顶部的"📱 同步微信号"按钮
4. 等待同步完成(约10-30秒)
5. 左侧微信号列表会自动刷新,显示所有微信号

**注意事项**:
- ✅ 首次使用必须先同步微信号,否则左侧列表为空
- ✅ 同步过程会自动登录堆雪球并提取微信号列表
- ✅ 如果堆雪球账号中的微信号有变化,可以重新点击"同步微信号"按钮更新
- ✅ 本地测试时可以设置`PUPPETEER_HEADLESS=false`查看浏览器操作过程

#### 第三步: 同步好友列表

1. 在左侧微信号列表中,选择需要同步的微信号(可多选)
2. 点击"同步好友"按钮
3. 等待同步完成(根据好友数量,可能需要5-30分钟)
4. 右侧会显示所有好友列表
...
在实现图片发送功能时,遇到了"发送文件"确认框的"确定"按钮无法点击的问题。
...
堆雪球的"发送文件"确认框**不是Element UI的对话框结构**,之前的代码只查找`.el-dialog__wrapper`,导致无法找到确定按钮。

#### 解决方案

**使用通用的按钮查找策略,不依赖特定的对话框结构**
...
1. **不依赖特定结构** - 不假设对话框是Element UI或其他特定框架
2. **z-index判断** - 通过z-index确保点击最上层的按钮,避免点击错误的按钮
3. **可见性检查** - 检查按钮及其所有父元素的可见性
4. **文本匹配** - 支持"确定"和"确 定"(中间有空格)两种格式
...
// 4. 点击确认框的确定按钮(使用通用查找策略)
const confirmClicked = await page.evaluate(() => {
  // 查找所有可见的"确定"按钮,选择z-index最大的
  // ... (上面的代码)
});
