...
- **随机延迟** - 每条消息发送前可添加随机延迟,模拟真人操作
- **双重匹配** - 素材匹配支持URL和文字双重匹配,提高成功率
- **🆕 智能表情** - 文字消息自动添加随机表情,防止被识别为批量操作
...
- **特性**:
  - 支持`{昵称}`变量个性化
  - 🆕 **自动添加随机表情** - 每条消息首尾自动添加1-2个随机表情
  - 🆕 **智能防重复** - 基于核心文案(去除表情)判断是否重复
- **示例**:
  - 原文案: "你好{昵称},这是一条测试消息"
  - 实际发送: "😊 你好张三,这是一条测试消息 🎉" (表情随机)
- **适用场景**: 日常问候、活动通知、产品推广
...
**功能概述**: 为文字消息自动添加随机表情,防止被微信识别为批量操作,同时保持防重复发送机制正常工作。

#### 核心原理

**双层设计**:
1. **发送层**: 实际发送的消息带有随机表情
2. **去重层**: 防重复检查时只比对核心文案(去除表情)
...
**表情组合数计算**:
- 首尾各1个: 100 × 100 = **10,000种组合**
- 首尾各2个: 100² × 100² = **1亿种组合**
- 混合模式(1-2个随机): 约 **2500万种组合**

**对于12万好友**: 平均每种组合只用0.0048次,**几乎不会重复!** 🎉
...
private extractCoreText(text: string): string {
  ...
  // 去除开头的表情符号和空白(支持所有Unicode表情)
  coreText = coreText.replace(/^[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}...]+/gu, '');
...
| 发送次数 | 实际发送内容 | 核心文案 | 哈希值 | 结果 |
|---------|------------|---------|--------|------|
| 第1次 | 😊 你好,这是测试 🎉 | 你好,这是测试 | abc123... | ✅ 发送成功 |
| 第2次 | 🌟 你好,这是测试 ✨ | 你好,这是测试 | abc123... | ⏭️ 跳过(已发送) |
| 第3次 | 💪👍 你好,这是测试 🔥💥 | 你好,这是测试 | abc123... | ⏭️ 跳过(已发送) |

**场景2: 不同核心文案**

| 发送次数 | 实际发送内容 | 核心文案 | 哈希值 | 结果 |
|---------|------------|---------|--------|------|
| 第1次 | 😊 你好,这是第一条 🎉 | 你好,这是第一条 | abc123... | ✅ 发送成功 |
| 第2次 | 😊 你好,这是第二条 🎉 | 你好,这是第二条 | def456... | ✅ 发送成功 |

#### 优势总结

1. **✅ 防封效果好**: 每条消息都不同,不会被识别为批量操作
2. **✅ 组合数充足**: 2500万种组合,12万好友几乎不重复
3. **✅ 防重复有效**: 基于核心文案判断,表情不影响去重
4. **✅ 无需配置**: 全自动,用户无需手动选择表情
5. **✅ 性能优秀**: 本地随机,无需调用AI API,速度快成本低
