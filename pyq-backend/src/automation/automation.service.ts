...
export class AutomationService {
  ...
  async script1_LinkAutoPublish(
    ...
    try {
      this.gateway.emitScript1Log(taskId, '🚀 开始执行自动发布流程...');
      this.gateway.emitScript1Log(taskId, `📝 文章链接: ${url}`);

      // 步骤1: 自动采集文章内容和图片
      this.gateway.emitScript1Log(taskId, '📥 [步骤1/5] 正在采集文章内容...');
      const articleData = await this.collectionService.extractArticle(url);
      this.gateway.emitScript1Log(taskId, `✅ [步骤1/5] 采集成功: ${articleData.title}`);

      // 步骤2: 自动调用Coze AI转写
      this.gateway.emitScript1Log(taskId, '🤖 [步骤2/5] 正在调用Coze AI转写文案...');
      const rewriteResult = await this.cozeService.rewriteContent(
        articleData.content,
        userId,
      );
      this.gateway.emitScript1Log(taskId, '✅ [步骤2/5] 转写成功');

      // 步骤3: 自动选择版本1创建发布任务
      this.gateway.emitScript1Log(taskId, '📋 [步骤3/5] 正在创建发布任务...');
      ...
      // 步骤5: 创建发布任务
      this.gateway.emitScript1Log(taskId, '📤 [步骤5/5] 正在创建发布任务...');
      ...
      const publishTask = await this.publishService.createTask({
        ...
        contentType: options.contentType || 'image', // 默认图文类型
      ...
      this.gateway.emitScript1Log(taskId, `✅ [步骤5/5] 发布任务创建成功: ${publishTask.id}`);
      ...
      this.gateway.emitScript1Log(taskId, `❌ 执行失败: ${error.message}`);
      this.gateway.emitScript1Complete(taskId, false, error.message);
